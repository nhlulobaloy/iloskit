...
// Fetch all user orders (ordered by latest)
...
<style>
:root { --primary-color: #6c63ff; --secondary-color: #f8f9fa; --accent-color: #ff6b6b; }
body { background-color: #f5f7fa; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
.order-card { border: none; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); transition: transform 0.3s ease, box-shadow 0.3s ease; margin-bottom: 25px; overflow: hidden; }
.order-card:hover { transform: translateY(-5px); box-shadow: 0 10px 25px rgba(0,0,0,0.1); }
.order-header { background-color: var(--primary-color); color: white; padding: 15px 20px; border-bottom: none; }
...
.back-btn { background-color:var(--primary-color); color:white; border:none; padding:10px 20px; border-radius:30px; transition:all 0.3s ease; }
.back-btn:hover { background-color:#5a52d6; transform:translateX(-5px); }
.empty-state { text-align:center; padding:40px; background-color:white; border-radius:10px; box-shadow:0 4px 15px rgba(0,0,0,0.05); }
.empty-state i { font-size:5rem; color:#d1d5db; margin-bottom:20px; }
.status-note { font-size:0.9rem; color:rgb(0,0,0); font-weight:600; margin-top:8px; background-color:rgb(228,228,228); padding:8px 12px; border-radius:6px; max-width:fit-content; }
</style>
</head>
<body class="p-4">
<div class="container">
...
<a href="index.php" class="btn back-btn"><i class="fas fa-arrow-left me-2"></i> Back to Home</a>
</div>
...
<?php if ($ordersResult->num_rows === 0): ?>
    <div class="empty-state">
        ...
        <a href="index.php" class="btn btn-primary px-4 py-2"><i class="fas fa-shopping-bag me-2"></i> Start Shopping</a>
    </div>
<?php else: ?>
    <?php while ($order = $ordersResult->fetch_assoc()): ?>
        <div class="card order-card mb-4">
            <div class="order-header d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0">Order #<?= htmlspecialchars($order['order_id'] ?: $order['id']) ?></h5>
                ...
                <span class="status-badge status-<?= strtolower(htmlspecialchars($order['status'])) ?>"><?= ucfirst(htmlspecialchars($order['status'])) ?></span>
            </div>
            <div class="card-body">
                <div class="row mb-4">
                    <div class="col-md-4">
                ...
                <?php
                $orderKey = $order['order_id'] ?: $order['id']; // fallback
                    ...
                    <table class="table">
                        ...
                        <?php while($item = $itemsResult->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>R<?= number_format($item['price_at_purchase']*(int)$item['quantity'],2) ?></td>
